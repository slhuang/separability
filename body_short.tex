...
\new{We note that feature pair extraction are widely adopted in the literature and has been shown to be effective in many different biological settings~\cite{hanczar2007feature,geman2004classifying,shi2011top, shen2017identification,sinha2008systematic, watkinson2008identification}. In particular, \cite{geman2004classifying, shi2011top} demonstrate that predictors with gene-pairs as input features are more robust to normalization and can achieve better model performance, compared to predictors with only single genes as features. However, }
...
\new{Most existing works~\cite{shen2017identification, basu2018iterative,shah2014random,schwarz2010safari} employ machine learning approaches, heuristically returning feature pairs without really exploring the full search space. To the contrary, \genviz aims to provide optimal results by exhaustively searching over the full space, while employing a suit of optimizations to speed up the whole process. Specifically, }
...
\new{Other metrics, e.g., synergy in \cite{watkinson2008identification}, have been proposed for evaluating the utility of feature pairs. However, such metrics can hardly benefit from optimization techniques and thus result in a very long running time even with parallel computing as reported in \cite{watkinson2008identification}. Indeed, similar metric as in \genviz, has been adopted in \cite{unger2010linear} for separating two classes of biological samples, however, with no focus on improving the efficiency~\cite{unger2010linear}.}
