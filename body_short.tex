...
accentuate or explain the differences between them\new{---a problem we refer to as the {\em separability problem}.}
...
\new{To balance between the two above-mentioned ``extreme'' strategies, people have historically looked at feature pairs, which is more informative than single features and not as complex as general machine learning algorithms. Existing works~\cite{hanczar2007feature,geman2004classifying,shi2011top, shen2017identification,sinha2008systematic, watkinson2008identification} have shown that feature pair extraction is an effective approach in many different biological settings. In particular, \cite{geman2004classifying, shi2011top} demonstrate that predictors using gene-pairs as input features are more robust to normalization and can achieve better model performance, compared to predictors using only single genes as features. To identify top feature pairs, some existing works~\cite{shen2017identification, basu2018iterative,shah2014random,schwarz2010safari} employ machine learning approaches, heuristically returning feature pairs without really exploring the full search space. As a result of heuristic, there is no guarantee on the quality of the returned feature pairs. On the other hand, other works~\cite{watkinson2008identification} aim to provide optimal results by exhaustively searching over the full space. However, they do not scale to a large number of features and samples. This is partially due to the selection of the separability metric. For instance, synergy is proposed in \cite{watkinson2008identification} for evaluating the utility of feature pairs. Metrics like synergy aim to capture not only linear separability but also non-linear separability. Consequently, the intrinsic complexity of these metrics makes them difficult to benefit from optimization techniques and thus result in a very long running time even with parallel computing as reported in \cite{watkinson2008identification}. To the contrary, metrics based on linear separability has limited coverage of different separability scenarios, but is more intuitive to the users and in the meantime enables optimizations. Indeed, using linear separability metric to identify pairs of genomic features between two classes of biological samples has been studied previously. For example, Unger \& Chor \cite{unger2010linear} sought to identify all gene pairs whose expression levels achieve perfect separation between two classes. Similarly, Sinha~\cite{sinha2003discriminative} proposed methods to identify pairs of motifs that discriminate two classes of sequences. However, none of them focus on improving the efficiency.}


%Indeed, \cite{unger2010linear} adopted linear separability metric for their problem, but with no focus on how to improve the efficiency~\cite{unger2010linear}


%{Motivated by these observations, we sought to build a tool that strikes a balance between the two above-mentioned ``extreme'' strategies, enabling us to retrieve more than just the best {\em singular} features, and doing so in an {\em interactive} manner.}

Motivated by these observations, we present \genviz,
...
\end{table}
 ...
 \begin{tabular}{|p{2.3cm}|p{1.2cm}|p{1.2cm}|p{1.5cm}|p{1.2cm}|p{1.2cm}|p{1.5cm}|} %|c|c|c|c|c||c|c|
  ...
  \hline
  & EARTY- STOP & SAMP- LING & Candidate Ordering & TRAVER- SAL & Approx- mation & Complex- tity\\
