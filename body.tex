...
The bottom half of Figure~\ref{fig:earlyT} shows $\widehat{\mm}$ after object reordering where ``problematic'' objects are placed on the left. Here, after checking only the first two objects, we can terminate and eliminate this feature pair from further consideration.
\end{example}
...
We have developed \genviz to rapidly identify pairs of features
that best separate biological objects of two classes.
This enables researchers to visualize and explore the
interplay between important genomic features
rather than simply returning a top ranking list,
while still allowing for the instantaneous discovery
of the feature pairs that would be prohibitively expensive
via complex machine learning feature extraction methods.
To overcome the computational burden that comes with examining feature pairs from high dimensional datasets, we identified a number of interoperable optimizations,
including \trans and \earlyT to remove unnecessary calculations,
\sampling to quickly estimate separability,
and different \traversal routines to efficiently order and prune candidate feature pairs.
These optimization modules led to a $180\times$ and $400\times$ speedup when
returning the top feature pairs on our two benchmark datasets, \msig and \lincs.
The quality of the top feature pairs returned by \genviz on the \lincs dataset
...
At the core of \genviz is the Rocchio-based separability measure.
This linear separator, based on the perpendicular bisector of
the centroids of the two object classes, not only provides a
simple score for separability that matches our intuition,
but it also well approximates the best possible linear separator
quickly and enables optimizations like \trans that can pre-compute
important quantities from the feature-object matrix before the positive
and negative object sets are even provided.
One potential downside of the Rocchio-based measure is
that it is only designed to find feature pairs where the
object class distributions are distinct
and convex and isotropic around their centroids.
However, feature pairs with equally distinct
object class distributions that form arbitrary,
irregular patterns are potentially interesting,
but will not be found by \genviz. The Rocchio-based measure is
unable to discover these feature pairs that demonstrate more complex,
separating relationships between the objects.

Finally, because there are computationally prohibitive number of
feature pairs to consider, \genviz has a number of optimization
modules that sacrifice feature pair ranking accuracy for performance.
The \sampling module and the \traversal modules make stochastic
or greedy decisions in order to estimate the quality of
and prune the potential candidate feature pairs.
In both of the benchmarking sets, we observe that for our settings,
the sacrifice in accuracy was slight for the \sampOpt feature pair rankings
and more severe when using the \horiz and \vertic rankings
with the greedy candidate traversal.
Users of \genviz can explore the trade-off with performance
and accuracy by modifying the sample size, $|\sss|$,
used by the \sampling module or the number of
 candidate feature pairs examined, $\chi$,
 by \traversal module depending on the needs of their research and dataset.
