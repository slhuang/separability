...
\stitle{Enhancement by Feature Ordering.} We propose to first sort features based on single feature's separability score. Single features performs good by their own, i.e., with high separability score, are ranked in the front. Let $\{f_1^{'} f_2^{'},\cdots,f_m^{'}\}$ be the sorted single feature set. First, we coarsely classify the sorted feature set into three categories based on the single feature separability score: {\em top, median,} and {\em bad}. Next, we discuss the intuition behind different traversal order:
