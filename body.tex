...
This reflects a {\bf \em 180X} and {\bf \em 400X} speedup \sinha{I dont think journals allow bolding. Even italicizing is allowed only for terminology and definitions, I believe, not for added emphasis.}
...
\cb{move to permanent/accessible location/domain} and is open-sourced at \url{https://github.com/KnowEnG/Genvisage}.
\silu{add screenshot}
...
Given a feature pair $(f_i,f_j)$ as axes, we can visualize the object sets $\oo_+$ and $\oo_-$ in a 2-D space, where each object corresponds to a point with x-value and y-value as the object's value on feature $f_i$ and $f_j$ respectively. A desirable (i.e., both interesting and interpretable) visualization would be one in which the objects are {\em linearly separated}, defined as follows. Two sets of objects, i.e., $\oo_+$ and $\oo_-$, are said to be \emph{linearly separable} \needcite{cite a textbook, since linear separability is a standard topic and we're only introducing it for clarity of subsequent text.} if there exists at least one straight line in the 2-D space such that all points from $\oo_+$ are on one side of the line while all points from $\oo_-$ are on the other side.
