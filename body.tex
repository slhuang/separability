...
We demonstrate that the feature pairs identified by \genviz often more significantly
...
visualized in a two-dimensional interface, which with its intuitive layout users
can easily interpret the different relationships among the features between the two classes of objects.
...
With a separability measure in hand, we then develop a suite of novel
...
\sinha{is there another work we can cite that has done this?} \silu{Aditya's another paper~\cite{parameswaran2012crowdscreen} did similar trick, but in different problem. Do we want to cite it?}
...
Note that this step only involves one addition and one comparison.
...
\stitle{High Level Idea.} We maintain a upper bound
...
\stitle{High Level Idea.} \sampling primarily consists of two phases,
...
\tr{\cb{still need to be certain on the steps required to create this matrix, also may want to mention restart prob, etc}
\sinha{About the details: providing them here might be a distraction, but the Methods section is also not appropriate because it has a nice narrative structure to it rather than the usual dump of methodological details. Maybe the citation to DRaWR is sufficient for details?}}
...
\stitle{Algorithms.}
We evaluated six combinations of our optimization modules \sinha{"modules" is somewhat misleading since it suggests separation during combination, while in reality I believe a combination of modules can weave together two ideas to the point that they are no longer "modular". Maybe add a clarification here that what we call modules really are "strategies" for optimization. Or replace "modules" with "strategies" throughout. This is under the assumption that I am right about modularity being lost during some combinations.}
...
\stitle{Takeaways.} If the accuracy is paramount to the user,
...
\centering %%% not \center
\vspace{-5mm}
\subfigure[\msig]{\label{fig:msig_singleF}\includegraphics[width=.16\textwidth]{fig/msig_SF_FP.pdf}}
\subfigure[\lincs]{\label{fig:lincs_singleF}\includegraphics[width=.31\textwidth]{fig/lincs_SF_FP.pdf}}
\vspace{-5mm}
...
We have developed the \genviz algorithm to rapidly identify the pairs of features from genomic data that best separate the objects of two classes. This enables researchers to visualize and explore the interplay between important genomic features rather than simply returning a top ranking list, while still allowing for the real time discovery of the features pairs that would be prohibitive from more complex machine learning feature extraction methods. In order to overcome the computational burden that comes with examining feature pairs from high dimensional datasets, we combined a number of important algorithm optimizations which included \trans and \earlyT to remove unnecessary calculations, \sampling to quickly estimate separability, and different \traversal routines to efficiently order and prune candidate feature pairs. These optimization modules led to a 180x and 400x speedup in the task of returning the top feature pairs on our two benchmark datasets, \msig and \lincs. The quality of the top feature pairs returned by \genviz on the \lincs dataset was confirmed by their agreement with literature and interaction databases, and the features are easily understood with a simple heatmap visualization.

At the core of the \genviz algorithm is the Rocchio-based separability measure. This linear separator, based on the perpendicular bisector of the centroids of the two object classes, not only provides a simple score for separability that matches our intuition, but it also well approximates the best possible linear separator quickly and enables optimizations like \trans that can pre-compute important quantities from the feature-object matrix before the positive and negative object sets are even provided. One potential downside of relying on the Rocchio-based measure is that it is only designed to find feature pairs where the object class distributions are distinct and Gaussian around their centroids. However, feature pairs with equally distinct object class distributions that form arbitrary, non-Gaussian patterns are potentially interesting, but will not be found by \genviz. The Rocchio-based measure is unable to discover these feature pairs that demonstrate more complex, separating relationships between the objects.

Finally, because there are computationally prohibitive number of possible feature pairs to consider, \genviz has a number of optimization modes that sacrifice feature pair ranking accuracy for performance. The \sampling module and the \traversal modules make stochastic or greedy decisions in order to estimate the quality of and prune the potential candidate feature pairs. In both of the benchmarking sets, we observe that for our settings of the parameters, the sacrifice in accuracy was slight for the \sampOpt feature pair rankings and more severe when using the \horiz and \vertic rankings with the greedy candidate traversal. Users of \genviz can explore the trade-off with performance and accuracy by modifying the sample size, $|\sss|$, used by the \sampling module or the number of candidate feature pairs examined, $\chi$, by \traversal module depending on the needs of their research and dataset.
...
This work was supported by the National Institutes of Health Big Data to Knowledge (BD2K) initiative [1U54GM114838].\vspace*{-12pt}
...
\label{tbl:supplement1}
\label{tbl:supplement2}
