...
visualized in a two-dimensional interface, \silu{which is the most intuitive visualization to human perception. 
Thus, users can easily interpret how one object class is separated from the other object class, by looking at how these two classes of objects are scattered across the two-dimensional visualization.}
...
\agp{Charles: too complex a sentence. Split or simplify.}
...
\cb{still need to be certain on the steps required to create this matrix, also may want to mention restart prob, etc} 
...
\stitle{Algorithms.} 
We evaluated six combinations of our optimization modules 
...
the 10 datasets in \msig, for which the average number of positive \silu{total} objects was 295.
\sinha{Is the notation correct? Isnt 295 the average number of *positive* objects? } \cb{silu, I am also slightly confused. did we do the brute force comparison with pos vs neg msig or pos vs all msig?} \silu{pos vs neg msig}
...
\cb{we need more clarity (maybe in the discussion) why the early termination is included if it is not performing well, i.e. cases where we would recommend using it } \silu{in most cases, it is performing better than the baseline. If in comparison with other algorithms, early termination has better separability quality.}
...
\label{fig:viz}
\end{figure}
\cb{silu, i think the colors in the figure are reversed from the legend. if so, can you change the figure? red for negative is more natural of bioinformatics} \silu{updated, please check}
